 ...
 * @type {string}
 */
md.redis.resp.CRLF = md.redis.resp.Sign.PRETERMINAL +
    md.redis.resp.Sign.TERMINAL;
...
md.redis.resp.encodeCommand = function(args) {
  ...
  if (l > 0) {
    var i = 0;
    command = md.redis.resp.DataType.ARRAY + String(l) + md.redis.resp.CRLF;

    while (i < l) {
      command += md.redis.resp.DataType.BULK +
          Buffer.byteLength(args[i]) + md.redis.resp.CRLF +
          String(args[i]) + md.redis.resp.CRLF;
      i += 1;
    }
  }
...
md.redis.resp.HEADER_PARSER = function(packet, cursor, chunk) {
  ...
  if (packetType === md.redis.resp.DataType.ARRAY ||
      packetType === md.redis.resp.DataType.BULK) {
...
md.redis.resp.BODY_PARSER = function(packet, cursor, chunk) {
  ...
  if (length === -1) {
    ...
    } else if (packetType === md.redis.resp.DataType.BULK) {
