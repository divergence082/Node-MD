...
md.redis.resp.Packet.prototype.get = function() {
  ...
  switch (this.__type) {
    case md.redis.resp.Terminal.BULK:
    case md.redis.resp.Terminal.STRING:
    case md.redis.resp.Terminal.ERROR:
      return (this.__data ? this.__data.toString() : '');

    case md.redis.resp.Terminal.INTEGER:
...
md.redis.resp.Packet.prototype.isError = function() {
  return this.__type === md.redis.resp.Terminal.ERROR;
};
...
/**
 * @param {md.redis.resp.Terminal} packetType
 */
...
md.redis.resp.Packet.prototype.hasHeader = function() {
  return (this.__type !== md.redis.resp.Terminal.UNDEFINED &&
      this.__length !== -1);
};
